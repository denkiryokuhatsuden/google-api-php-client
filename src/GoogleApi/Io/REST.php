...
class REST
{
    ...
    public static function decodeHttpResponse ($response)
    {
        ...
        if ($code != '200' && $code != '201' && $code != '204') {
            ...
            if ($decoded != null && isset($decoded['error']['message']) &&
                ...
                // if we're getting a json encoded error definition, use that
        ...
        // Only attempt to decode the response, if the response code wasn't
        // (204) 'no content'
        if ($code != '204') {
            $decoded = json_decode($body, true);
            if ($decoded == null) {
    ...
    static function createRequestUri ($basePath, $restPath, $params)
    {
        ...
        foreach ($params as $paramName => $paramSpec) {
            // Discovery v1.0 puts the canonical location under the 'location'
            // field.
