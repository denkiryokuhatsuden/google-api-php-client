...
class OAuth2 extends Auth
{
    ...
    private function refreshTokenRequest ($params)
    {
        ...
        if (200 == $code) {
            ...
            throw new Exception(
                    "Error refreshing the OAuth2 token, message: '$body'", $code);
        }
    }
    ...
    /**
     * Verifies the id token, returns the verified token contents. Visible for testing.
     * 
     * @param unknown_type $jwt
     * @param unknown_type $certs
     * @param unknown_type $required_audience
     * @throws Exception
     * @return \GoogleApi\Auth\LoginTicket
     */
    public function verifySignedJwtWithCerts ($jwt, $certs, $required_audience)
    {
        ...
        if ($exp >= $now + self::MAX_TOKEN_LIFETIME_SECS) {
            throw new Exception("Expiration time too far in future: $json_body");
        }
        
        $latest = $exp + self::CLOCK_SKEW_SECS;
        if ($now < $earliest) {
            throw new Exception("Token used too early, $now < $earliest: $json_body");
        }
        if ($now > $latest) {
            throw new Exception("Token used too late, $now > $latest: $json_body");
        }
